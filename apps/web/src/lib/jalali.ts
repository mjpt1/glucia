// Jalali (Shamsi) ↔ Gregorian conversion — jalaali-js algorithm (MIT).
...
/** Current Jalali year as a number (e.g. 1405). */
export function currentJalaliYear(): number {
  return Number(new Intl.DateTimeFormat('fa-IR-u-nu-latn', { year: 'numeric' }).format(new Date()));
}

/** Build a JS Date from Jalali date + time (local timezone). */
